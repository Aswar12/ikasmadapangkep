...
php artisan migrate:status | findstr "add_profile_photo"
if %errorlevel% equ 0 (
...
php -r "require 'vendor/autoload.php'; $app = require_once 'bootstrap/app.php'; $app->make('Illuminate\Contracts\Console\Kernel')->bootstrap(); use Illuminate\Support\Facades\Schema; if (Schema::hasTable('profiles')) { echo '   - Profiles table exists' . PHP_EOL; if (Schema::hasColumn('profiles', 'profile_photo')) { echo '   - profile_photo column exists' . PHP_EOL; } else { echo '   - profile_photo column NOT found!' . PHP_EOL; } } else { echo '   - Profiles table NOT found!' . PHP_EOL; }"
